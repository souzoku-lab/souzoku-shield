...
def evaluate_bad_demo_fixture(rules: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    """具体的なfault injectionに対して、ハーネスがREDを出せることを示す。"""
    bad_results = []
    for case in rules["expert"]["harness_cases"]:
        passed, detail = _run_check(case["check"], rules, state, reducer=_faulty_reduce_for)
        potential_damage_yen = int(case.get("damage_yen", 0))
        monetary = potential_damage_yen > 0
        bad_results.append(
            {
                ...
                "monetary": monetary,
                "impact_label": case.get("impact_label", "否認インパクト" if monetary else "○×確認"),
                "detail": f"fault injection: {detail}",
            }
        )
    total_damage = sum(item["damage_yen"] for item in bad_results)
    return {
        "ok": all(item["passed"] for item in bad_results),
        "total_damage_yen": total_damage,
        "impact_note": rules["expert"]["small_residence_denial_impact"]["summary"],
        "tax_formula_note": rules["expert"]["small_residence_denial_impact"]["tax_formula_note"],
        "results": bad_results,
...
def _run_check(
    ...
    state: dict[str, Any],
    *,
    reducer: Callable[[str, dict[str, Any], dict[str, Any]], dict[str, Any]] = None,
) -> tuple[bool, str]:
    reduce_for = reducer or _reduce_for
    checks: dict[str, Callable[[dict[str, Any], dict[str, Any]], tuple[bool, str]]] = {
        "spouse_must_not_require_continuation": lambda r, s: _check_spouse_no_continuation(r, s, reduce_for),
        "co_resident_must_require_continuation": lambda r, s: _check_co_resident_continuation(r, s, reduce_for),
        "house_lost_must_require_2018_docs": lambda r, s: _check_house_lost_2018(r, s, reduce_for),
        "draft_must_not_use_definitive_phrases": lambda r, s: _check_no_definitive(r, s, reduce_for),
        "overall_opinion_must_be_blank": lambda r, s: _check_overall_blank(r, s, reduce_for),
...
def _faulty_reduce_for(acquirer_type: str, rules: dict[str, Any], state: dict[str, Any]) -> dict[str, Any]:
    """RED証拠用に、実際に起こり得る誤実装を出力へ注入する。"""
    reduced = copy.deepcopy(_reduce_for(acquirer_type, rules, state))
    required = reduced["acquirer"]["required_document_ids"]
    if acquirer_type == "spouse" and "co_resident_continuation" not in required:
        required.append("co_resident_continuation")
    if acquirer_type == "co_resident":
        reduced["acquirer"]["required_document_ids"] = [
            doc_id for doc_id in required if doc_id != "co_resident_continuation"
        ]
    if acquirer_type == "house_lost":
        reduced["acquirer"]["required_document_ids"] = [
            doc_id for doc_id in required if doc_id != "house_lost_no_home_docs"
        ]
        reduced["draft"]["section_3_land_review"] = [
            line.replace("2018年改正後の", "") for line in reduced["draft"]["section_3_land_review"]
        ]
    forbidden = rules["expert"]["safety_rules"]["forbidden_definitive_phrases"][0]
    reduced["draft"]["section_3_land_review"].append(forbidden)
    reduced["draft"]["section_5_overall_opinion"] = "AIが自動入力した総合所見"
    return reduced


def _check_spouse_no_continuation(
    rules: dict[str, Any],
    state: dict[str, Any],
    reduce_for: Callable[[str, dict[str, Any], dict[str, Any]], dict[str, Any]],
) -> tuple[bool, str]:
    reduced = reduce_for("spouse", rules, state)
...
def _check_co_resident_continuation(
    rules: dict[str, Any],
    state: dict[str, Any],
    reduce_for: Callable[[str, dict[str, Any], dict[str, Any]], dict[str, Any]],
) -> tuple[bool, str]:
    reduced = reduce_for("co_resident", rules, state)
    required = set(reduced["acquirer"]["required_document_ids"])
    passed = "co_resident_continuation" in required
    return passed, "同居親族の継続確認資料を確認"


def _check_house_lost_2018(
    rules: dict[str, Any],
    state: dict[str, Any],
    reduce_for: Callable[[str, dict[str, Any], dict[str, Any]], dict[str, Any]],
) -> tuple[bool, str]:
    reduced = reduce_for("house_lost", rules, state)
    required = set(reduced["acquirer"]["required_document_ids"])
...
def _check_no_definitive(
    ...
    for acquirer_type in rules["expert"]["acquirer_types"]:
        reduced = reduce_for(acquirer_type, rules, state)
